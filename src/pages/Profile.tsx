    ...
    const fetchAll = async () => {
      try {
        ...
        const [profileRes, projectsRes, pubsRes, cvsRes] = await Promise.allSettled([
          ...
          api.get('/projects/?my_projects_only=true'),
          api.get<Publication[]>('/publications/'),
          api.get<StudentCV[]>('/student-cvs/'),
        ]);
...
const handleAddProject = async () => {
  ...
  try {
    ...
    const res = await api.post<Publication>('/publications/', {
      ...publicationForm,
  ...
  const handleUploadCV = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await api.post<StudentCV>('/student-cvs/', {
        ...cvForm,
  ...
  const handleDeleteCV = async (id: number) => {
    ...
    try {
      await api.delete(`/student-cvs/${id}`);
  ...
  const handleSaveSettings = async () => {
    try {
      setSubmitting(true);
      const res = await api.put<UserProfile>('/auth/me', settingsForm);
      setProfile(res.data);
...
const infoItems = [
  ...
  profile?.department ? { icon: <UserIcon />, value: profile.department } : null,
  profile?.phone_number ? { icon: <Phone />, value: profile.phone_number } : null,
  profile?.address ? { icon: <MapPin />, value: profile.address } : null,
].filter(Boolean);
  return (
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white p-8 rounded-[3rem] border border-brand-navy/5 shadow-2xl relative overflow-hidden text-center">
              ...
              <div className="space-y-3 text-left border-t border-brand-navy/5 pt-6">
                ...
                <span className="w-4 h-4 mr-3 text-brand-orange flex-shrink-0">
                  {item.icon}
                 </span>
